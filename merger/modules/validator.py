...
from ..exceptions import ModuleValidationError, IncompatibleModulesError
...
class ModuleValidator:
    ...
    def _validate_persona_level_module(self, df: pd.DataFrame, module_code: str) -> List[str]:
        ...
        if 'codperso' in df.columns:
            ...
            if invalid_codperso > 0:
                warnings.append(f"Módulo {module_code}: {invalid_codperso} códigos de persona inválidos")
            ...
            if pd.api.types.is_numeric_dtype(df['codperso']):
                ...
                if max_codperso > 30:
                    warnings.append(f"Módulo {module_code}: código de persona muy alto ({max_codperso})")
    ...
    def _validate_hogar_level_module(self, df: pd.DataFrame, module_code: str,
                                     required_cols: List[str]) -> List[str]:
        ...
        if 'hogar' in df.columns and not any(col for col in required_cols if col not in df.columns):
            ...
            if multi_records > 0:
                warnings.append(f"Módulo {module_code}: {multi_records} hogares con múltiples registros")
    ...
    def _validate_special_module(self, df: pd.DataFrame, module_code: str) -> List[str]:
        ...
        if module_code == '37':  # Gobierno electrónico
            ...
            if df.empty:
                warnings.append("Módulo 37: módulo vacío (normal si no hay datos de gobierno electrónico)")
    ...
    def _validate_sumaria_module(self, df: pd.DataFrame) -> List[str]:
        ...
        if 'mieperho' in df.columns:
            ...
            if invalid_members.any():
                warnings.append(f"Sumaria: {invalid_members.sum()} hogares con número de miembros inválido")
        ...
        if 'gashog2d' in df.columns and 'inghog2d' in df.columns:
            ...
            inconsistent = (df['gashog2d'] > df['inghog2d'] * 2) & \
                           (df['gashog2d'] > 0) & (df['inghog2d'] > 0)
            if inconsistent.any():
                warnings.append(f"Sumaria: {inconsistent.sum()} hogares con gastos muy superiores a ingresos")
    ...
    def _validate_economic_module(self, df: pd.DataFrame, module_code: str) -> List[str]:
        ...
        amount_cols = [col for col in df.columns if any(keyword in col.lower()
                                                        for keyword in ['monto', 'gasto', 'ingreso', 'valor'])]
            ...
            for col in amount_cols:
                if pd.api.types.is_numeric_dtype(df[col]):
                    ...
                    if negative_count > len(df) * 0.1:  # Más del 10%
                        warnings.append(f"Módulo {module_code}: {negative_count} valores negativos en '{col}'")

                    # Verificar valores extremos
                    if df[col].max() > 1000000:  # Más de 1M
                        extreme_count = (df[col] > 1000000).sum()
                        warnings.append(f"Módulo {module_code}: {extreme_count} valores extremos en '{col}' (>1M)")

        return warnings

    def check_module_compatibility(self, df1: pd.DataFrame, df2: pd.DataFrame,
        ...
        compatibility_matrix = {
            ...
            (ModuleType.HOGAR_LEVEL, ModuleType.PERSONA_LEVEL): merge_level == ModuleMergeLevel.HOGAR,
            (ModuleType.PERSONA_LEVEL, ModuleType.HOGAR_LEVEL): merge_level == ModuleMergeLevel.HOGAR,
    ...
    def _detailed_compatibility_analysis(self, df1: pd.DataFrame, df2: pd.DataFrame,
                                         merge_keys: List[str]) -> Dict[str, Any]:
    ...
    def validate_data_consistency(self, df: pd.DataFrame, module_code: str) -> Dict[str, Any]:
        ...
        if module_code in self.config.module_validations:
            ...
            if missing_keys:
                ...
                if null_in_keys > 0:
                    report['issues_found'].append(f"Valores nulos en llaves: {null_in_keys} registros")
        ...
        for col in numeric_cols:
            if df[col].min() < 0 and col.lower() not in ['ingreso', 'monto', 'valor']:
                ...
                negative_count = (df[col] < 0).sum()
                if negative_count > 0:
                    report['issues_found'].append(f"Valores negativos inesperados en '{col}': {negative_count}")
