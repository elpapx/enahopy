...
class ENAHONullAnalyzer:
    ...
    def __init__(self, config: Optional[NullAnalysisConfig] = None):
        ...
        self.pattern_analyzer = NullPatternAnalyzer(self.pattern_detector) if PATTERNS_AVAILABLE else None
    ...
    def get_summary(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            'null_values': df.isnull().sum().sum(),
            'null_percentage': (df.isnull().sum().sum() / df.size) * 100,
            'columns_with_nulls': find_columns_with_nulls(df) if UTILS_AVAILABLE else df.columns[df.isnull().any()].tolist(),
...
def analyze_null_patterns(df: pd.DataFrame, 
                         config: Optional[NullAnalysisConfig] = None) -> Dict[str, Any]:
    """
...
def generate_null_report(df: pd.DataFrame,
                        ...
                        format: str = 'html',
                        include_visualizations: bool = True) -> Any:
    """
