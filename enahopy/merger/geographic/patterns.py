...
from ..config import NivelTerritorial, PATRONES_GEOGRAFICOS
...
class GeoPatternDetector:
    ...
    def detectar_columnas_geograficas(self, df: pd.DataFrame,
                                      confianza_minima: float = 0.8) -> Dict[str, str]:
        """
        ...
        for tipo_geo, patrones in PATRONES_GEOGRAFICOS.items():
            ...
            if mejor_coincidencia:
                ...
                self.logger.info(f"Detectado {tipo_geo}: '{mejor_coincidencia}' (confianza: {mejor_score:.1f})")

        # Guardar en cache
        self._detection_cache[cache_key] = columnas_detectadas
        return columnas_detectadas

    def _calculate_similarity_score(self, patron: str, columna_lower: str,
                                    df: pd.DataFrame, columna_original: str) -> float:
        """
    ...
    def analyze_geographic_completeness(self, df: pd.DataFrame,
                                        columnas_detectadas: Dict[str, str]) -> Dict[str, float]:
        """
    ...
    def detect_geographic_patterns_in_data(self, df: pd.DataFrame,
                                           columnas_geo: Dict[str, str]) -> Dict[str, any]:
        """
        ...
        if 'ubigeo' in columnas_geo and columnas_geo['ubigeo'] in df.columns:
            ...
            patrones['distribucion_territorial']['departamentos'] = {
                ...
                'departamento_mas_frecuente': dist_departamentos.index[0] if not dist_departamentos.empty else None,
                'concentracion_top3': dist_departamentos.head(3).sum() / len(df) * 100 if len(df) > 0 else 0
            }
        ...
        for nivel, columna in columnas_geo.items():
            if columna in df.columns:
                ...
                patrones['cobertura_por_nivel'][nivel] = {
                    ...
                    'diversidad_relativa': valores_unicos / valores_totales if valores_totales > 0 else 0
                }

        return patrones

    def suggest_merge_strategy(self, df1: pd.DataFrame, df2: pd.DataFrame,
                               columnas_geo1: Dict[str, str],
                               columnas_geo2: Dict[str, str]) -> Dict[str, any]:
        """
        ...
        for nivel in niveles_comunes:
            ...
            if col1 in df1.columns and col2 in df2.columns:
                score = self._evaluate_merge_compatibility(
                    df1[col1], df2[col2], nivel
                )
    ...
    def _evaluate_merge_compatibility(self, serie1: pd.Series, serie2: pd.Series,
                                      nivel: str) -> float:
        """
    ...
    def _compare_format_consistency(self, serie1: pd.Series, serie2: pd.Series,
                                    nivel: str) -> float:
    ...
    def generate_detection_report(self, df: pd.DataFrame,
                                  columnas_detectadas: Dict[str, str]) -> str:
        """
        ...
        lines.extend([
            f"🎯 NIVEL TERRITORIAL SUGERIDO: {nivel_sugerido.value.upper()}",
            ""
        ])
