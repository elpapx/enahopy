...
def safe_dict_merge(dict1: Optional[Dict], dict2: Optional[Dict],
                    strategy: str = "update") -> Dict:
    """
...
def merge_analysis_results(results: List[Dict[str, Any]],
                           aggregation_strategy: str = "average") -> Dict[str, Any]:
    """
...
class InputValidator:
    ...
    @staticmethod
    def validate_geographic_filter(filter_dict: Optional[Dict[str, str]]) -> Optional[Dict[str, str]]:
        """
...
def safe_enum_conversion(value: Union[str, Any],
                         enum_class: type,
        ...
        if not is_valid:
            ...
            if coerce:
                try:
                    ...
                    validation['conversion_errors'].append({
                        'column': col,
                        'error': str(e)
                    })
...
@contextmanager
def null_analysis_context(operation_name: str,
                          logger=None,
    ...
    try:
        ...
        if logger:
            ...
            logger.info(
                f"Completado {status}: {operation_name} "
                f"({context['elapsed_time']:.2f}s)"
            )
...
def format_percentage(value: float, decimals: int = 1,
...
def format_large_number(value: Union[int, float],
                        use_abbreviation: bool = True) -> str:
    """
