...
class VisualizationFactory:
    ...
    def create_basic_visualizations(self, analysis_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if (self.config.visualization_type in [VisualizationType.INTERACTIVE, VisualizationType.BOTH]
    ...
    def _create_missing_bar_chart(self, analysis_result: Dict[str, Any]) -> plt.Figure:
        ...
        bars1 = ax1.barh(summary_with_missing['variable'],
                        summary_with_missing['missing_percentage'],
                        color=sns.color_palette(self.config.color_palette,
                                               len(summary_with_missing)))
        ...
        bars2 = ax2.barh(summary_with_missing['variable'],
        ...
        for i, bar in enumerate(bars2):
            width = bar.get_width()
            ax2.text(width + len(str(int(width))) * 100, bar.get_y() + bar.get_height() / 2,
    ...
    def _create_missing_heatmap(self, analysis_result: Dict[str, Any]) -> plt.Figure:
        ...
        if 'correlations' in analysis_result:
            ...
            ax.set_title('Correlación entre Patrones de Valores Faltantes',
                        fontweight='bold', pad=20)
        else:
            ax.text(0.5, 0.5, 'Correlaciones no disponibles',
    ...
    def _create_correlation_matrix(self, analysis_result: Dict[str, Any]) -> plt.Figure:
        ...
        if 'correlation_matrix' not in correlations:
            fig, ax = plt.subplots(figsize=self.config.figure_size)
            ax.text(0.5, 0.5, 'Matriz de correlaciones no disponible',
                   ha='center', va='center', transform=ax.transAxes)
            return fig
        ...
        ax.set_title('Correlaciones entre Patrones de Valores Faltantes',
                    fontweight='bold', pad=20)
    ...
    def _create_interactive_missing_chart(self, analysis_result: Dict[str, Any]) -> go.Figure:
        ...
        fig = make_subplots(
            rows=2, cols=1,
            subplot_titles=('Porcentaje de Valores Faltantes',
    ...
    def _create_missing_patterns_sunburst(self, analysis_result: Dict[str, Any]) -> go.Figure:
        ...
        fig.update_layout(
            title="Patrones Más Comunes de Valores Faltantes",
